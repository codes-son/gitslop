...
import { Github, ExternalLink, GitMerge, MessageSquare, Zap } from "lucide-react";
...
export function GifCard({ meme, index }: GifCardProps) {
  const delay = Math.min(index * 0.06, 1.0);
  ...
  return (
    <motion.div
      ...
      className="mb-6 rounded-2xl overflow-hidden glass-panel group relative flex flex-col"
    >
      <a
        href={meme.githubUrl}
        target="_blank"
      ...
      <div className="relative bg-zinc-950 overflow-hidden">
        ...
        <div className="absolute top-3 left-3 z-30 pointer-events-none">
          <div className="bg-black/70 backdrop-blur-md px-2.5 py-1.5 rounded-lg border border-white/10 flex items-center gap-1.5 shadow-lg">
            ...
            <span className="text-[10px] font-bold text-yellow-300 uppercase tracking-wider">GIF</span>
          </div>
        </div>
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-yellow-400/90 text-black rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 scale-75 group-hover:scale-100 transition-all duration-300 shadow-xl shadow-yellow-400/30 pointer-events-none z-30">
          <ExternalLink className="w-5 h-5 ml-0.5" />
        </div>
      </div>

      <div className="p-4 sm:p-5 flex flex-col gap-3 relative z-30 pointer-events-none">
        <h3 className="font-mono text-sm font-bold text-yellow-400 truncate">
          &gt; {meme.keyword}
        </h3>
